...
def convert_cwr_to_amazon(input_file: Path, output_file: Path, mapping: dict,
                           delimiter: str = '\t') -> int:
    """Convert CWR inventory file to Amazon multi-location CSV.
    ...
    with output_file.open('w', newline='', encoding='utf-8') as outf:
        writer = csv.DictWriter(outf,
                                fieldnames=['sku', 'fulfillment-center-id', 'quantity'])
        writer.writeheader()
        processed = 0
        for row in rows:
            ...
            for col, source_id in mapping.items():
                ...
                writer.writerow({
                    'sku': sku,
                    'fulfillment-center-id': source_id,
                    'quantity': qty
                })
            writer.writerow({'sku': sku, 'fulfillment-center-id': 'DEFAULT', 'quantity': 0})
            processed += 1

...
class KeystoneSupplier(Supplier):
  ...
  <soapenv:Body>
    ...
    def fetch_inventory_secondary(self, progress_cb: 'Callable[[int], None] | None' = None) -> None:
        """Retrieve the inventory update file via FTP.

        Parameters
        ----------
        progress_cb: callable, optional
            If supplied, will be invoked with an integer percentage (0-100)
            as the download progresses so callers (e.g. Flask API) can expose
        ...
        try:
            if progress_cb:
                progress_cb(0)  # Always start at 0%
            with open(output, 'wb') as f:
                def write_chunk(data):
                    print("Chunk received, len:", len(data))  # debug
                    ...
                    if total:
                        ...
                        percent = min(99, max(1, downloaded // (200 * 1024)))
            ...
            if progress_cb and last_percent < 100:
                progress_cb(100)  # Always finish at 100%
            if total:

...
import { fetchCatalogRows, deleteSku, deleteCatalogFile } from '../api/seawide'
...
export default function CatalogManager() {
  ...
  const load = (p: number = 0) => {
    setLoading(true);
    fetchCatalogRows({ limit: pageSize + 1, start: p * pageSize })
      .then((r) => {
  ...
  // You may want to adjust these fields for Seawide if needed
  const qtyFields = [
  ...
  return (
    <div className="space-y-4">
                ...
                <React.Fragment key={i}>
                  <tr className={`border-t border-gray-200 hover:bg-gray-50 ${isOpen ? 'bg-blue-50' : ''}`} onClick={() => setExpanded(isOpen ? null : i)}>
                    ...
                    <td className="px-4 py-2 text-right">
                      <button onClick={e => { e.stopPropagation(); deleteSku(sku).then(()=>load()); }} className="rounded p-1 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
                    </td>
                  </tr>
                  {isOpen && (
      ...
      <div className="flex justify-between items-center mt-2">
        <button onClick={prevPage} disabled={page === 0} className="px-3 py-1 rounded bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50">Previous</button>
        <span className="text-sm text-gray-600">Page {page + 1}</span>
        <button onClick={nextPage} disabled={!hasNext} className="px-3 py-1 rounded bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50">Next</button>
      </div>
    </div>
  )
}